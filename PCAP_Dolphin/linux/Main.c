/* PCAP Dolphin - a simple and fast TCP/UDP PCAP Filter
...
#define MAGIC_NUMBER_02 0xA1B2C34D
...
const char* Initialize_Output_File(const char* input_file)
{
	...
	// Check filetype
	if (!(strcmp(input_file + input_length - 5, ".pcap") == 0 || strcmp(input_file + input_length - 5, ".PCAP")))
	{
		printf("File not a .pcap file. Bailing out! \n Filename: %s\n", "input_file");
#ifdef _WIN32
		system("pause");
#endif
		exit(1);
...
int main(int argc, char* argv[])
{
	...
	unsigned int Magic_Number_in_file = *((int*)buffer);
	if (!(Magic_Number_in_file == MAGIC_NUMBER_01 || Magic_Number_in_file == MAGIC_NUMBER_02))
	{
		printf("Magic Numbers not correct. %x \n bailing out!", Magic_Number_in_file);
		return 1;
	}
	...
	{
		// check if header is atleast loaded in until captured packet length and if not load in
		if ((current_block + 1) * BLOCK_SIZE < (long long)(pos + 12))
		{
#ifdef _WIN32
			s_load = clock();
#endif
		...
		unsigned int packet_length = *((unsigned int*)(buffer + BLOCK_POSITION(pos+8)));
		
		// Load in new block if not the whole message is already in RAM
		...
		int filters_matched = Classify_Package(pos, buffer);
		...
		if (filters_matched)
		{
			...
			if (BLOCK_POSITION(pos) > BLOCK_POSITION(pos + packet_length))
			{
				int Bytes_in_second_block = 2 * BLOCK_SIZE - BLOCK_POSITION(pos);
				fwrite((void*)(buffer + BLOCK_POSITION(pos)), 1, Bytes_in_second_block, f_out);
				fwrite(buffer, 1, 16 + packet_length - Bytes_in_second_block, f_out);
			}
			else
				fwrite(buffer + BLOCK_POSITION(pos), 1, 16 + packet_length, f_out);
		...
		// Jump to next packet
		//skip all bytes of current packege + packet lengths (4Bytes) +  timestamp (sec) of new packet (4Bytes) + timestamp (micro/nanoseconds) of new packet (4Bytes)
		pos += packet_length + 16;
